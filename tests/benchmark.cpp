...
int main() {
    ...
    // encryption
    unsigned char aes128Key[16] = {0xde, 0xed, 0xbe, 0xef, 0xca, 0xfe, 0xba, 0xbe,
                                   0xfe, 0xed, 0xbe, 0xef, 0xde, 0xad, 0xde, 0xed};

    unsigned char aes192Key[24] = {0xde, 0xed, 0xbe, 0xef, 0xca, 0xfe, 0xba, 0xbe, 0xfe, 0xed, 0xbe, 0xef,
                                   0xde, 0xad, 0xde, 0xed, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07};

    unsigned char aes256Key[32] = {0xde, 0xed, 0xbe, 0xef, 0xca, 0xfe, 0xba, 0xbe, 0xfe, 0xed, 0xbe,
                                   0xef, 0xde, 0xad, 0xde, 0xed, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
                                   0x06, 0x07, 0xff, 0x00, 0xff, 0x00, 0xff, 0x00, 0xff, 0xbb};
    ...
    // ECB
    {
        ...
        std::cout << std::fixed << std::setprecision(2) << "| AES128 Enc | ECB | " << MB << " | "
                  << (double) aes128EncDur.count() / nanosec_in_sec << " | " << aes128EncSpeed * nanosec_in_sec
                  << " mb/s | N/A |\n";

        std::cout << std::fixed << std::setprecision(2) << "| AES128 Dec | ECB | " << MB << " | "
                  << (double) aes128DecDur.count() / nanosec_in_sec << " | " << aes128DecSpeed * nanosec_in_sec
                  << " mb/s | " << aes128res << " |\n";
        ...
        std::cout << std::fixed << std::setprecision(2) << "| AES192 Enc | ECB | " << MB << " | "
                  << (double) aes192EncDur.count() / nanosec_in_sec << " | " << aes192EncSpeed * nanosec_in_sec
                  << " mb/s | N/A |\n";

        std::cout << std::fixed << std::setprecision(2) << "| AES192 Dec | ECB | " << MB << " | "
                  << (double) aes192DecDur.count() / nanosec_in_sec << " | " << aes192DecSpeed * nanosec_in_sec
                  << " mb/s | " << aes192res << " |\n";
        ...
        std::cout << std::fixed << std::setprecision(2) << "| AES256 Enc | ECB | " << MB << " | "
                  << (double) aes256EncDur.count() / nanosec_in_sec << " | " << aes256EncSpeed * nanosec_in_sec
                  << " mb/s | N/A |\n";

        std::cout << std::fixed << std::setprecision(2) << "| AES256 Dec | ECB | " << MB << " | "
                  << (double) aes256DecDur.count() / nanosec_in_sec << " | " << aes256DecSpeed * nanosec_in_sec
                  << " mb/s | " << aes256res << " |\n";
    }
    ...
    // CBC
    {
        ...
        std::cout << std::fixed << std::setprecision(2) << "| AES128 Enc | CBC | " << MB << " | "
                  << (double) aes128EncDur.count() / nanosec_in_sec << " | " << aes128EncSpeed * nanosec_in_sec
                  << " mb/s | N/A |\n";

        std::cout << std::fixed << std::setprecision(2) << "| AES128 Dec | CBC | " << MB << " | "
                  << (double) aes128DecDur.count() / nanosec_in_sec << " | " << aes128DecSpeed * nanosec_in_sec
                  << " mb/s | " << aes128res << " |\n";
        ...
        std::cout << std::fixed << std::setprecision(2) << "| AES192 Enc | CBC | " << MB << " | "
                  << (double) aes192EncDur.count() / nanosec_in_sec << " | " << aes192EncSpeed * nanosec_in_sec
                  << " mb/s | N/A |\n";

        std::cout << std::fixed << std::setprecision(2) << "| AES192 Dec | CBC | " << MB << " | "
                  << (double) aes192DecDur.count() / nanosec_in_sec << " | " << aes192DecSpeed * nanosec_in_sec
                  << " mb/s | " << aes192res << " |\n";
        ...
        std::cout << std::fixed << std::setprecision(2) << "| AES256 Enc | CBC | " << MB << " | "
                  << (double) aes256EncDur.count() / nanosec_in_sec << " | " << aes256EncSpeed * nanosec_in_sec
                  << " mb/s | N/A |\n";

        std::cout << std::fixed << std::setprecision(2) << "| AES256 Dec | CBC | " << MB << " | "
                  << (double) aes256DecDur.count() / nanosec_in_sec << " | " << aes256DecSpeed * nanosec_in_sec
                  << " mb/s | " << aes256res << " |\n";
    }
    ...
    // CFB
    {
        ...
        std::cout << std::fixed << std::setprecision(2) << "| AES128 Enc | CFB | " << MB << " | "
                  << (double) aes128EncDur.count() / nanosec_in_sec << " | " << aes128EncSpeed * nanosec_in_sec
                  << " mb/s | N/A |\n";

        std::cout << std::fixed << std::setprecision(2) << "| AES128 Dec | CFB | " << MB << " | "
                  << (double) aes128DecDur.count() / nanosec_in_sec << " | " << aes128DecSpeed * nanosec_in_sec
                  << " mb/s | " << aes128res << " |\n";
        ...
        std::cout << std::fixed << std::setprecision(2) << "| AES192 Enc | CFB | " << MB << " | "
                  << (double) aes192EncDur.count() / nanosec_in_sec << " | " << aes192EncSpeed * nanosec_in_sec
                  << " mb/s | N/A |\n";

        std::cout << std::fixed << std::setprecision(2) << "| AES192 Dec | CFB | " << MB << " | "
                  << (double) aes192DecDur.count() / nanosec_in_sec << " | " << aes192DecSpeed * nanosec_in_sec
                  << " mb/s | " << aes192res << " |\n";
        ...
        std::cout << std::fixed << std::setprecision(2) << "| AES256 Enc | CFB | " << MB << " | "
                  << (double) aes256EncDur.count() / nanosec_in_sec << " | " << aes256EncSpeed * nanosec_in_sec
                  << " mb/s | N/A |\n";

        std::cout << std::fixed << std::setprecision(2) << "| AES256 Dec | CFB | " << MB << " | "
                  << (double) aes256DecDur.count() / nanosec_in_sec << " | " << aes256DecSpeed * nanosec_in_sec
                  << " mb/s | " << aes256res << " |\n";
    }
